...
export const spec: OpenAPIV3_1.Document = {
  ...
  components: {
    ...
    schemas: {
      ...
      AAEEvalResult: { type: 'object', additionalProperties: true, description: 'AAE permission evaluation verdict.',
        properties: { decision: { type: 'string', enum: ['permit','deny','indeterminate'] }, reasons: { type: 'array', items: { type: 'string' } }, constraints: { type: 'object', additionalProperties: true } } },
      AAEInfo: { type: 'object', additionalProperties: true, description: 'AAE-evaluation service info: schema URI, supported operations.' },
      GovernanceValidateResult: { type: 'object', additionalProperties: true,
        description: 'Capability-validation verdict (used by aeoess + MolTrust agents). Spend-ceiling + score-gate.',
        properties: { decision: { type: 'string', enum: ['permit','deny'] }, score: { type: 'number' }, reasons: { type: 'array', items: { type: 'string' } } } },
      ActionCheckResult: { type: 'object', additionalProperties: true,
  ...
  paths: {
    ...
    '/api/action/check': {
      post: { tags: ['aae-evaluation'], summary: 'Check whether an action is permitted for a DID (free)',
        description: 'Pre-action gate — evaluates AAE + flags + risk profile.',
        operationId: 'checkAction',
        requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', additionalProperties: true } } } },
        responses: { '200': { description: 'Check result', content: { 'application/json': { schema: { '$ref': '#/components/schemas/ActionCheckResult' } } } } } },
    },
    '/api/action/events/{did}': {
      get: { tags: ['aae-evaluation'], summary: 'List action events for a DID', operationId: 'getActionEvents',
        parameters: [{ name: 'did', in: 'path', required: true, schema: { type: 'string' } }],
        responses: { '200': { description: 'Events', content: { 'application/json': { schema: { '$ref': '#/components/schemas/ActionEvents' } } } } } },
    },
    '/api/action/stats': {
      get: { tags: ['aae-evaluation'], summary: 'Aggregate action-check statistics', operationId: 'getActionStats',
        responses: { '200': { description: 'Stats', content: { 'application/json': { schema: { '$ref': '#/components/schemas/ActionStats' } } } } } },
    },
    '/governance/validate-capabilities': {
      post: { tags: ['aae-evaluation'], summary: 'Validate an agent\'s authorized capabilities (free)',
        description: 'Used by aeoess + MolTrust cross-verify pipeline. Returns permit/deny + score + reasons.',
        operationId: 'validateCapabilities',
        requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', additionalProperties: true } } } },
        responses: { '200': { description: 'Validation verdict', content: { 'application/json': { schema: { '$ref': '#/components/schemas/GovernanceValidateResult' } } } } } },
    },
    '/vc/aae/evaluate': {
      get: { tags: ['aae-evaluation'], summary: 'Evaluate AAE — GET variant (query-driven)', operationId: 'evaluateAAEGet',
        responses: { '200': { description: 'AAE evaluation', content: { 'application/json': { schema: { '$ref': '#/components/schemas/AAEEvalResult' } } } } } },
      post: { tags: ['aae-evaluation'], summary: 'Evaluate AAE — POST variant (body-driven)', operationId: 'evaluateAAEPost',
        requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', additionalProperties: true } } } },
        responses: { '200': { description: 'AAE evaluation', content: { 'application/json': { schema: { '$ref': '#/components/schemas/AAEEvalResult' } } } } } },
    },
    '/vc/aae/info': {
